...
function HackCard({ hack, index, t }) {
        ...
        {hack.guide && (
          <div>
            ...
            >
              ...
              {expanded
                ? (ts.guideTitle.includes('Qanday') ? 'Yashirish' : 'Hide')
                : ts.guideTitle}
            </button>
            <AnimatePresence>
              {expanded && (
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <section className="relative pt-28 pb-32 px-4 overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute inset-0 bg-gradient-to-b from-blue-950/90 via-blue-950/80 to-gray-950" />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {hacks.map((hack, i) => (
              <HackCard key={hack.id} hack={hack} index={i} t={t} />
            ))}
          </div>
        )}
      </section>
        ...
        >
          ...
          <div className="relative z-10">
            <h3 className="text-2xl font-bold text-white mb-3">
              {lang === 'uz' ? 'Pro maslahat' : 'Pro Tip'}
            </h3>
            <p className="text-blue-200 max-w-lg mx-auto text-sm leading-relaxed">
              {lang === 'uz'
                ? 'Eng samarali yo\'l — Spaced Repetition + Shadowing kombinatsiyasi. Har kuni 30 daqiqa shadowing va 20 ta yangi so\'z Anki bilan. 3 oyda natija ko\'rinadi!'
                : 'The most effective combination — Spaced Repetition + Shadowing. 30 minutes of shadowing daily + 20 new words with Anki. Results visible in 3 months!'}
            </p>
          </div>
        </motion.div>
      </section>
